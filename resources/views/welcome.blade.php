<x-guestLayout>
<head>
    <link rel="stylesheet" href="{{ asset('css/CG.css') }}">
</head>
<body>

    <!-- Introdução -->
    <section class="intro">
        <h1>What2Watch</h1>
        <p>Onde o seu entertenimento é o nosso dever!</p>
    </section>

    <!-- Seção 1: Texto à esquerda, imagem à direita -->
    <section class="section">
        <div class="text">
            <h2> <!-- O filme do dia é {{ $film->title }}</h2> -->
            <p>O meu principal Objetivo é criar um site onde o utilizador possa descobrir novos filmes e series e dar a sua opiniao.</p>
            <p>Nota a imagem tem de ser do genero de pessoas a assistirem algo</p>
        </div>
        <video width="400" controls>
            <source src="imagens/video/video.mp4" type="video/mp4">
            <source src="imagens/video/video.ogg" type="video/ogg">
            Your browser does not support HTML video.
          </video>
    </section>
    <section class="section">
        <div class="image">
            <img src="images/imagem2.jpg" alt="telemovel tecnologico">
        </div>
        <div class="text">
            <h2>Futuras Updates</h2>
            <p>Melhorar o site e criar uma app facil (a imagem tem de de um telemovel).</p>
        </div>
    </section>
    <section class="section">
        <div class="text">
            <h2>Objetivo do Site</h2>
            <p>O meu principal Objetivo é criar um site onde o utilizador possa descobrir novos filmes e series e dar a sua opiniao.</p>
            <p>Nota a imagem tem de ser do genero de pessoas a assistirem algo</p>
        </div>
        <div class="image">
            <img src="images/imagem1.jpg" alt="Imagem 1">
        </div>
    </section>
    
</body>

</x-guestLayout>