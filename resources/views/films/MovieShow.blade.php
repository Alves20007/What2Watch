    <x-guestLayout>
        ...
        @php
                // Se a relação estiver carregada, usamos diretamente
                $actors = $film->relationLoaded('actors') || method_exists($film, 'actors')
                    ? $film->actors
                    : collect();

                // Caso a relação não esteja disponível ou esteja vazia, tentamos buscar por IDs armazenados no campo Elenco
                if ($actors->isEmpty()) {
                    $actorIds = is_string($film->Elenco) ? json_decode($film->Elenco, true) : [];
                    $actorIds = is_array($actorIds) ? $actorIds : [];
                    $actors = count($actorIds) > 0
                        ? \App\Models\Actor::whereIn('id', $actorIds)->get()
                        : collect();
                }
            @endphp
            <!-- Banner e dados principais -->
            <section class="relative bg-neutral-700 text-white mb-6">
                <div class="absolute inset-0">
                    <img src="/bannerFilme/{{ empty($film->banner) || $film->banner === 'N/A' ? 'banner.jpg' : $film->banner }}"
                        alt="{{ $film->title }}"
                ...
                <div class="relative z-10 px-6 py-10 max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-6">
                    ...
                    @if($film->tipo === 'serie')
                            ...
                            <p class="mb-2"><b>Criador:</b> {{ $film->Diretor ?: 'Desconhecido' }}</p>
                        @elseif($film->tipo === 'filme')
                        ...
                        @elseif($film->tipo === 'video')
                            <p><b>Criador:</b> {{ $film->creator->UserName ?? 'Desconhecido' }}</p>
                        @endif
                    ...
                    @foreach($actors as $actor)
                            <a href="/atores/{{ $actor->Slug }}" class="min-w-[200px] max-w-[200px] bg-neutral-800 rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition duration-300 group">
                                <div class="h-64 w-full overflow-hidden">
                                    <img src="/imagens/atores/{{ $actor->image }}" alt="{{ $actor->Name }}"
                                ...
                                <div class="p-4">
                                    ...
                                    <p class="text-sm text-yellow-300 italic">
                                        Personagem: {{ $actor->pivot->personagem ?? 'Desconhecido' }}
                                    </p>
                                </div>
                            </a>
                        @endforeach
            ...
            <section class="p-6">
                ...
                @if ($film->reviews->count() > 0)
                    <p class="mt-4 text-sm text-gray-300 italic">Média: <span class="font-bold">{{ number_format($film->reviews->avg('rating'), 1) }}/5</span>
                        ({{ $film->reviews->count() }} reviews)</p>
                    <div class="mt-4 space-y-4">
                        @foreach ($film->reviews as $review)
                            <div class="p-3 bg-neutral-800 rounded shadow">
                                <p class="font-semibold mt-4 text-sm text-gray-300 italic">{{ $review->user->name }} — Nota: {{ $review->rating }}/5</p>
                                <p class="text-neutral-100 mt-4 text-sm text-gray-300 italic">{{ $review->comment }}</p>
                            </div>
                        @endforeach
                    </div>
                @else
                    <p class="mt-4 text-sm text-gray-300 italic">Ainda não há reviews. Sê o primeiro a avaliar!</p>
                @endif
